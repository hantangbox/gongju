...
namespace RivenSharp
{
    class RivenSharp
    {
        ...
        private static void onLoad(EventArgs args)
        {
            ...
            Config = new Menu("鐟炴枃 Sharp", "Riven", true);
            //Orbwalkervar menu = new Menu("My Mainmenu", "my_mainmenu", true);
            var orbwalkerMenu = new Menu("LX 璧扮爫", "my_Orbwalker");
            LXOrbwalker.AddToMenu(orbwalkerMenu);
            Config.AddSubMenu(orbwalkerMenu);
           //TS
           var TargetSelectorMenu = new Menu("鐩爣 閫夋嫨", "Target Selector");
           SimpleTs.AddToMenu(TargetSelectorMenu);
           Config.AddSubMenu(TargetSelectorMenu);
            //Combo
            Config.AddSubMenu(new Menu("杩炴嫑 Sharp", "combo"));
            Config.SubMenu("combo").AddItem(new MenuItem("forceQE", " 鍏坾E 鍚巪Q")).SetValue(true);
            Config.SubMenu("combo").AddItem(new MenuItem("packets", "浣跨敤 灏佸寘")).SetValue(true);

            //Haras
            Config.AddSubMenu(new Menu("楠氭壈 Sharp", "haras"));
            Config.SubMenu("haras").AddItem(new MenuItem("doHarasE", "浣跨敤 E 楠氭壈")).SetValue(new KeyBind('G', KeyBindType.Press, false));
            Config.SubMenu("haras").AddItem(new MenuItem("doHarasQ", "浣跨敤 Q 楠氭壈")).SetValue(new KeyBind('T', KeyBindType.Press, false));

            //Drawing
            Config.AddSubMenu(new Menu("鑼冨洿 Sharp", "draw"));
            Config.SubMenu("draw").AddItem(new MenuItem("doDraw", "绂佺敤 鑼冨洿")).SetValue(false);
            Config.SubMenu("draw").AddItem(new MenuItem("drawHp", "鏄剧ず 浼ゅ 琛€閲弢")).SetValue(true);

            //Debug
            Config.AddSubMenu(new Menu("璋冭瘯 Sharp", "debug"));
            Config.SubMenu("debug").AddItem(new MenuItem("db_targ", "璋冭瘯 鐩爣")).SetValue(new KeyBind('0', KeyBindType.Press, false));
