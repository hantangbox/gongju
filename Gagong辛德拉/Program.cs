...
namespace GagongSyndra
{
    class Program
    {
        ...
        static void Game_OnGameLoad(EventArgs args)
        {
            ...
            //Base menu
            Menu = new Menu("|Gagong|杈涘痉鎷墊", "GagongSyndra", true);
            ...
            Menu.SubMenu("Harass").AddItem(new MenuItem("HarassAAQ", "浣跨敤Q鐨勫悓鏃惰蛋A").SetValue(false));
            ...
            //Farming menu:
            Menu.AddSubMenu(new Menu("娓呯嚎", "Farm"));
            Menu.SubMenu("Farm").AddItem(new MenuItem("UseQFarm", "浣跨敤 Q").SetValue(new StringList(new[] { "鎺х嚎", "娓呯嚎", "鍚屾椂", "绂佺敤"  }, 2)));
            Menu.SubMenu("Farm")
            .AddItem(
            new MenuItem("UseWFarm", "浣跨敤 W").SetValue(
            new StringList(new[] { "鎺х嚎", "娓呯嚎", "鍚屾椂", "绂佺敤"  }, 1)));
            Menu.SubMenu("Farm")
            .AddItem(
            new MenuItem("UseEFarm", "浣跨敤 E").SetValue(
            new StringList(new[] { "鎺х嚎", "娓呯嚎", "鍚屾椂", "绂佺敤" }, 3)));
            Menu.SubMenu("Farm")
            .AddItem(
            ...
            Menu.SubMenu("AutoKS").AddItem(new MenuItem("MaxE", "闂幇鍑绘潃|鏈€澶т汉鏁皘").SetValue(new Slider(2, 1, 5)));
            Menu.SubMenu("AutoKS").AddItem(new MenuItem("FKMANA", "濡傛灉钃濋噺瓒冲鍑绘潃浣跨敤闂幇").SetValue(false));
            ...
            Menu.SubMenu("Misc").AddItem(new MenuItem("Packets", "浣跨敤 灏佸寘").SetValue(false));
            Menu.SubMenu("Misc").AddItem(new MenuItem("IgniteALLCD", "鎵€鏈夋妧鑳藉湪CD|浣跨敤鐐圭噧").SetValue(false));

            //QE Setting
            Menu.AddSubMenu(new Menu("QE 璁剧疆", "QEsettings"));
            Menu.SubMenu("QEsettings").AddItem(new MenuItem("QEDelay", "QE 寤惰繜").SetValue(new Slider(0, 0, 150)));
            Menu.SubMenu("QEsettings").AddItem(new MenuItem("QEMR", "QE 鏋侀檺鑼冨洿 %").SetValue(new Slider(100, 0, 100)));
            Menu.SubMenu("QEsettings").AddItem(new MenuItem("UseQEC", "QE 榧犳爣鏂瑰悜鏁屼汉").SetValue(new KeyBind("T".ToCharArray()[0], KeyBindType.Press)));

            //R
            Menu.AddSubMenu(new Menu("R 璁剧疆", "Rsettings"));
            Menu.SubMenu("Rsettings").AddItem(new MenuItem("RallCD", "QWE鍦–D寮哄埗R").SetValue(true));
            ...
            Menu.SubMenu("Rsettings").AddSubMenu(new Menu("鐩爣鍙嚮鏉€浣跨敤R", "okR"));
            foreach (var enemy in ObjectManager.Get<Obj_AI_Hero>().Where(enemy => enemy.Team != Player.Team))
            ...
            Menu.SubMenu("Drawing").AddItem(new MenuItem("DrawWMAP", "W 鐩爣鍙傛暟").SetValue(true));
            Menu.SubMenu("Drawing").AddItem(new MenuItem("Gank", "鏁屼汉鍑绘潃鎻愮ず").SetValue(true));
            Menu.SubMenu("Drawing").AddItem(new MenuItem("DrawHPFill", "鏄剧ず杩炴嫑娑堣€楄閲弢").SetValue(true));
