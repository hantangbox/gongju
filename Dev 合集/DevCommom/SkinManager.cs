...
namespace DevCommom
{
    public class SkinManager
    {
        ...
        public void AddToMenu(ref Menu menu)
        {
            ...
            if (Skins.Count > 0)
            {
                Menu.AddSubMenu(new Menu("更換皮膚", "Skin Changer"));
                Menu.SubMenu("Skin Changer").AddItem(new MenuItem("Skin_" + ObjectManager.Player.ChampionName + "_enabled", "啟用 換膚").SetValue(true));
                Menu.SubMenu("Skin Changer").AddItem(new MenuItem("Skin_" + ObjectManager.Player.ChampionName + "_select", "皮膚").SetValue(new StringList(Skins.ToArray())));
