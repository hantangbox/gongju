...
namespace Assemblies.Champions {
    internal class Kalista : Champion {
        ...
        public Kalista() {
            ...
            Game.PrintChat("[Assemblies] - 澶嶄粐涔嬬煕鈥斿崱鑾変笣濉攟 鍔犺級鎴愬姛锛佹饥鍖朾y浜岀嫍锛丵Q缇361630847 .");
            Game.PrintChat(
        ...
        private void loadMenu() {
            ...
            menu.SubMenu("combo").AddItem(new MenuItem("useER", "使用 E 逃跑").SetValue(true));
            ...
            menu.SubMenu("laneclear").AddItem(new MenuItem("autoE", "自动 E 补刀").SetValue(true));
            menu.SubMenu("laneclear").AddItem(new MenuItem("eNum", "使用E|最少小兵数").SetValue(new Slider(3, 1, 10)));

            menu.AddSubMenu(new Menu("抢人头 设置", "killsteal"));
            menu.SubMenu("killsteal").AddItem(new MenuItem("useQK", "使用 Q 抢人头").SetValue(true));
            menu.SubMenu("killsteal").AddItem(new MenuItem("useEK", "使用 E 抢人头").SetValue(true));
            ...
            menu.SubMenu("flee").AddItem(new MenuItem("useAAF", "使用 平A跳跃 逃跑").SetValue(true));

            menu.AddSubMenu(new Menu("范围 设置", "drawing"));
            menu.SubMenu("drawing").AddItem(new MenuItem("drawQ", "显示 Q 范围").SetValue(false));
            menu.SubMenu("drawing").AddItem(new MenuItem("drawE", "显示 E 范围").SetValue(false));
            menu.SubMenu("drawing").AddItem(new MenuItem("drawStacks", "显示 矛 堆叠层数").SetValue(false));
            menu.SubMenu("drawing").AddItem(new MenuItem("drawFlee", "显示 逃跑 地点").SetValue(true));

            MenuItem dmgAfterComboItem = new MenuItem("DamageAfterCombo", "显示组合连招最大伤害").SetValue(true);
            ...
            menu.SubMenu("misc").AddItem(new MenuItem("eStacks", "使用E|矛堆叠层数").SetValue(new Slider(2, 1, 10)));
