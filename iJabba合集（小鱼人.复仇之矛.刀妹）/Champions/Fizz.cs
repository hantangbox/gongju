...
namespace Assemblies.Champions {
    internal class Fizz : Champion {
        ...
        public Fizz() {
            ...
            Game.PrintChat("[Assemblies] - 鑿插吂 - 娼睈娴风伒 鍔犺級鎴愬姛锛佹饥鍖朾y浜岀嫍锛丵Q缇361630847.");
        }
        ...
        private void loadMenu() {
            ...
            menu.SubMenu("combo").AddItem(
                new MenuItem("initR", "先手 使用 R").SetValue(new KeyBind("G".ToCharArray()[0], KeyBindType.Press)));
            menu.SubMenu("combo")
            ...
            menu.SubMenu("harass").AddItem(new MenuItem("useWH", "使用 W 骚扰").SetValue(false));
            menu.SubMenu("harass").AddItem(new MenuItem("eTower", "E回 附近 炮塔").SetValue(true));

            menu.AddSubMenu(new Menu("清线 设置", "laneclear"));
            menu.SubMenu("laneclear").AddItem(new MenuItem("useQL", "使用 Q 清线").SetValue(false));
            menu.SubMenu("laneclear").AddItem(new MenuItem("useEL", "使用 E 清线").SetValue(false));

            menu.AddSubMenu(new Menu("抢人头 设置", "killsteal"));
            menu.SubMenu("killsteal").AddItem(new MenuItem("useQKS", "使用 Q  抢人头").SetValue(true));
            menu.SubMenu("killsteal").AddItem(new MenuItem("useEKS", "使用 E  抢人头").SetValue(false));

            menu.AddSubMenu(new Menu("逃跑 设置", "flee"));
            menu.SubMenu("flee").AddItem(new MenuItem("useQFlee", "使用 Q 逃跑").SetValue(true));
            menu.SubMenu("flee").AddItem(new MenuItem("useEFlee", "使用 E 逃跑").SetValue(true));

            menu.AddSubMenu(new Menu("抢龙 设置", "steal"));
            menu.SubMenu("steal").AddItem(
                new MenuItem("stealKey", "抢大龙").SetValue(new KeyBind("T".ToCharArray()[0], KeyBindType.Press)));

            menu.AddSubMenu(new Menu("杂项 设置", "misc"));
            menu.SubMenu("misc").AddItem(new MenuItem("qWithR", "使用Q的同时使用R").SetValue(false));
            menu.SubMenu("misc").AddItem(new MenuItem("castEGap", "使用E 防止突进").SetValue(false));
            menu.SubMenu("misc").AddItem(new MenuItem("useDFG", "使用冥火连招").SetValue(true));

            menu.AddSubMenu(new Menu("显示 设置", "draw"));
            menu.SubMenu("draw").AddItem(
                new MenuItem("drawFlee", "显示 逃跑 位置").SetValue(new Circle(true, Color.Cyan)));
