...
namespace Marksman
{
    ...
    internal class Kalista : Champion
    {
        ...
        public Kalista()
        {
            Utils.PrintMessage("Kalista loaded 姹夊寲by浜岀嫍!QQ缇361630847.");
        ...
        public override bool HarassMenu(Menu config)
        {
            ...
            config.AddItem(
                new MenuItem("UseQTH" + Id, "浣跨敤 Q (鑷姩)").SetValue(new KeyBind("H".ToCharArray()[0],
                    KeyBindType.Toggle)));
            return true;
        }
        ...
        {
            config.AddItem(
                new MenuItem("JumpTo" + Id, "璺宠穬").SetValue(new KeyBind("T".ToCharArray()[0], KeyBindType.Press)));
            return true;
        }
        ...
        public override bool DrawingMenu(Menu config)
        {
            ...
            config.AddItem(
                new MenuItem("DrawConnMax" + Id, "杩炴帴 鑼冨洿").SetValue(new Circle(false,
                    Color.FromArgb(100, 255, 255, 255))));
            config.AddItem(
                new MenuItem("DrawConnText" + Id, "杩炴帴 鏂囨湰").SetValue(new Circle(false, Color.GreenYellow)));
            config.AddItem(
                new MenuItem("DrawConnSignal" + Id, "杩炴帴 淇″彿").SetValue(true));
            config.AddItem(new MenuItem("Dx" + Id, ""));
            config.AddItem(
                new MenuItem("DrawEStackCount" + Id, "E 鍙犲姞灞傛暟").SetValue(new Circle(false, Color.Firebrick)));
            config.AddItem(new MenuItem("Dx", ""));
            config.AddItem(
                new MenuItem("DrawJumpPos" + Id, "璺宠穬浣嶇疆").SetValue(new Circle(false, Color.HotPink)));

            var damageAfterE = new MenuItem("DamageAfterE", "鏄剧ず E 浼ゅ").SetValue(true);
