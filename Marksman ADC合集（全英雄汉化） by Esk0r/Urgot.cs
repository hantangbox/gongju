...
namespace Marksman
{
    internal class Urgot : Champion
    {
        ...
        public Urgot()
        {
            Utils.PrintMessage("Urgot loaded 姹夊寲by浜岀嫍!QQ缇361630847.");
        ...
        {
            config.AddItem(
                new MenuItem("DrawQ" + Id, "Q 鑼冨洿").SetValue(new Circle(true, Color.LightGray)));
            config.AddItem(
                new MenuItem("DrawE" + Id, "E 鑼冨洿").SetValue(new Circle(false, Color.LightGray)));
            config.AddItem(
                new MenuItem("DrawR" + Id, "R 鑼冨洿").SetValue(new Circle(false, Color.LightGray)));
            config.AddItem(
                new MenuItem("DrawQEx" + Id, "EQ浜岃繛鑼冨洿").SetValue(new Circle(true, Color.LightGray)));
            return true;
        }

        public override bool MiscMenu(Menu config)
        {
            config.AddItem(new MenuItem("UltOption1" + Id, "鑷姩R鍒板涓媩").SetValue(true));
            config.AddItem(new MenuItem("UltOption2" + Id, "鑷姩R鍒扮洘鍙嬩腑").SetValue(true));
            return true;
        }
    }
}