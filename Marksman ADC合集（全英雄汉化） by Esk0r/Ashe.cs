...
namespace Marksman
{
    ...
    internal class Ashe : Champion
    {
        ...
        public Ashe()
        {
            ...
            Utils.PrintMessage("Ashe loaded 姹夊寲by浜岀嫍!QQ缇361630847.");
        }
        ...
        public override bool HarassMenu(Menu config)
        {
            ...
            config.AddItem(
                new MenuItem("UseWTH" + Id, "浣跨敤 W (鎸佺画)").SetValue(new KeyBind("H".ToCharArray()[0],
                    KeyBindType.Toggle)));
            return true;
        }
        ...
        {
            config.AddItem(new MenuItem("DeactivateQ" + Id, "绂佺敤Q").SetValue(false));
            return true;
        }
        ...
        {
            config.AddItem(
                new MenuItem("DrawW" + Id, "W 鑼冨洿").SetValue(new Circle(true, Color.CornflowerBlue)));
            config.AddItem(
        ...
        {
            config.AddItem(new MenuItem("QExploit" + Id, "浣跨敤Q椋庣瓭").SetValue(true));
            config.AddItem(new MenuItem("RInterruptable" + Id, "鑷姩R鎵撴柇鎶€鑳絴").SetValue(true));
            config.AddItem(new MenuItem("EFlash" + Id, "瀵归棯鐜版柟浣嶆担璺ㄦ暏E").SetValue(true));
            config.AddItem(new MenuItem("RManualCast" + Id, "鎵嬪姩R(2000 鑼冨洿)"))
