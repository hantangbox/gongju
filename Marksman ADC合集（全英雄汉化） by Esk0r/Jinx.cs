...
namespace Marksman
{
    internal class Jinx : Champion
    {
        ...
        public Jinx()
        {
            Utils.PrintMessage("Jinx by [Credits in Github] loaded 姹夊寲by浜岀嫍!QQ缇361630847.");
        ...
        {
            config.AddItem(new MenuItem("SwapQ" + Id, "鎬绘槸鍒囨崲鏈烘灙").SetValue(false));
            return true;
        }

        public override bool MiscMenu(Menu config)
        {
            config.AddItem(new MenuItem("SwapDistance" + Id, "鏍规嵁璺濈鑷姩鍒囨崲Q").SetValue(true));
            config.AddItem(new MenuItem("SwapAOE" + Id, "鑷姩鍒囧ぇ鐐瓵OE").SetValue(false));
            config.AddItem(new MenuItem("MinWRange" + Id, "鏄剧ずW鏈€灏忚窛绂粅").SetValue(new Slider(525 + 65 * 2, 0, 1200)));
            config.AddItem(new MenuItem("AutoEI" + Id, "鑷姩E涓嶅姩鐨勬晫浜簗").SetValue(true));
            config.AddItem(new MenuItem("AutoES" + Id, "鑷姩E缂撴參鐨勬晫浜簗").SetValue(true));
            config.AddItem(new MenuItem("AutoED" + Id, "鑷姩E蹇€熺殑鏁屼汉|").SetValue(false));
            config.AddItem(
                new MenuItem("CastR" + Id, "鎵嬪姩R(2000 鑼冨洿)").SetValue(
                    new KeyBind("T".ToCharArray()[0], KeyBindType.Press)));
            config.AddItem(new MenuItem("ROverKill" + Id, "妫€娴婻鏄惁鑳藉嚮鏉€").SetValue(true));
            config.AddItem(new MenuItem("MinRRange" + Id, "鏈€灏廟鑼冨洿").SetValue(new Slider(300, 0, 1500)));
            config.AddItem(new MenuItem("MaxRRange" + Id, "鏈€澶鑼冨洿").SetValue(new Slider(1700, 0, 4000)));
            return true;
        }
        ...
        {
            config.AddItem(
                new MenuItem("DrawQBound" + Id, "鏄剧ずQ鏋侀檺鑼冨洿").SetValue(
                    new Circle(true, Color.FromArgb(100, 255, 0, 0))));
            config.AddItem(
                new MenuItem("DrawW" + Id, "W 鑼冨洿").SetValue(new Circle(false, Color.CornflowerBlue)));
            return true;
        }
