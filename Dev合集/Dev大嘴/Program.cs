    ...
    class Program
    {
        ...
        private static void onGameLoad(EventArgs args)
        {
            try
            {
                ...
                Game.PrintChat(string.Format("<font color='#fb762d'>DevKogMaw Loaded v{0}姹夊寲by浜岀嫍!QQ缇361630847</font>", Assembly.GetExecutingAssembly().GetName().Version));
        ...
        private static void InitializeMainMenu()
        {
            ...
            Config = new Menu("Dev澶у槾", "DevKogMaw", true);
            ...
            Config.SubMenu("Combo").AddItem(new MenuItem("RMaxStacksCombo", "R鏈€澶т娇鐢ㄦ鏁皘").SetValue(new Slider(3, 1, 5)));
            Config.SubMenu("Combo").AddItem(new MenuItem("UseIgnite", "浣跨敤鐐圭噧").SetValue(true));
            Config.SubMenu("Combo").AddItem(new MenuItem("EManaCombo", "浣跨敤E鏈€浣庤摑閲弢").SetValue(new Slider(20, 1, 100)));
            ...
            Config.SubMenu("Harass").AddItem(new MenuItem("UseRHarass", "浣跨敤 R").SetValue(true));
            Config.SubMenu("Harass").AddItem(new MenuItem("RMaxStacksHarass", "R鏈€澶т娇鐢ㄦ鏁皘").SetValue(new Slider(1, 1, 5)));
            Config.SubMenu("Harass").AddItem(new MenuItem("ManaHarass", "楠氭壈鏈€浣庤摑閲弢").SetValue(new Slider(25, 1, 100)));
            Config.SubMenu("Harass").AddItem(new MenuItem("EManaHarass", "浣跨敤E鏈€浣庤摑閲弢").SetValue(new Slider(60, 1, 100)));
            
            Config.AddSubMenu(new Menu("娓呯嚎", "LaneClear"));
            Config.SubMenu("LaneClear").AddItem(new MenuItem("UseELaneClear", "浣跨敤 E").SetValue(false));
            Config.SubMenu("LaneClear").AddItem(new MenuItem("EManaLaneClear", "浣跨敤E鏈€浣庤摑閲弢").SetValue(new Slider(50, 1, 100)));

            Config.AddSubMenu(new Menu("鎶汉澶磡", "KillSteal"));
            Config.SubMenu("KillSteal").AddItem(new MenuItem("RKillSteal", "浣跨敤R鎶汉澶磡").SetValue(true));

            Config.AddSubMenu(new Menu("鎶㈤噹锛堝ぇ灏忛緳绛夛級", "JungleSteal"));
            Config.SubMenu("JungleSteal").AddItem(new MenuItem("JungleStealAlert", "鎶㈤噹 璀﹁").SetValue(true));
            Config.SubMenu("JungleSteal").AddItem(new MenuItem("JungleSteal", "浣跨敤R鎶㈤噹").SetValue(true));

            Config.AddSubMenu(new Menu("鏉傞」", "Misc"));
            Config.SubMenu("Misc").AddItem(new MenuItem("ChaseEnemyAfterDeath", "鑷姩姝讳骸杩介€恷").SetValue(true));
            Config.SubMenu("Misc").AddItem(new MenuItem("PacketCast", "浣跨敤灏佸寘").SetValue(true));

            Config.AddSubMenu(new Menu("绐佽繘缁堢粨鑰厊", "GapCloser"));
            Config.SubMenu("GapCloser").AddItem(new MenuItem("BarrierGapCloser", "闃绘尅绐佽繘鑰厊").SetValue(true));
            Config.SubMenu("GapCloser").AddItem(new MenuItem("BarrierGapCloserMinHealth", "闃绘尅绐佽繘鑰厊鏈€浣庤閲弢").SetValue(new Slider(40, 0, 100)));
            Config.SubMenu("GapCloser").AddItem(new MenuItem("EGapCloser", "浣跨敤E闃叉绐佽繘").SetValue(true));
            Config.SubMenu("GapCloser").AddItem(new MenuItem("EInterrupt", "浣跨敤E涓柇娉曟湳").SetValue(true));

            Config.AddSubMenu(new Menu("R璁剧疆", "Ultimate"));
            Config.SubMenu("Ultimate").AddItem(new MenuItem("UseAssistedUlt", "浣跨敤R鏀彺闃熷弸").SetValue(true));
            Config.SubMenu("Ultimate").AddItem(new MenuItem("AssistedUltKey", "R鏀彺閿綅").SetValue((new KeyBind("R".ToCharArray()[0], KeyBindType.Press))));
            ...
            Config.SubMenu("Drawings").AddItem(new MenuItem("RDamage", "鏄剧ずR琛€閲忎激瀹硘").SetValue(true));
