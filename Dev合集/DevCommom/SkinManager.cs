...
namespace DevCommom
{
    public class SkinManager
    {
        ...
        public void AddToMenu(ref Menu menu)
        {
            ...
            if (Skins.Count > 0)
            {
                Menu.AddSubMenu(new Menu("鐨偆鏇存崲", "Skin Changer"));
                Menu.SubMenu("Skin Changer").AddItem(new MenuItem("Skin_" + ObjectManager.Player.ChampionName + "_enabled", "鍚敤鐨偆鏇存崲").SetValue(true));
                Menu.SubMenu("Skin Changer").AddItem(new MenuItem("Skin_" + ObjectManager.Player.ChampionName + "_select", "鐨偆閫夋嫨").SetValue(new StringList(Skins.ToArray())));
