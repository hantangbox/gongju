    ...
    class Program
    {
        ...
        static void Game_OnGameLoad(EventArgs args)
        {
            try
            {
                ...
                Game.PrintChat(string.Format("<font color='#fb762d'>DevRyze Loaded v{0}姹夊寲by浜岀嫍!QQ缇361630847</font>", Assembly.GetExecutingAssembly().GetName().Version));
        ...
        private static void InitializeMainMenu()
        {
            ...
            Config = new Menu("Dev鐟炲吂", "DevRyze", true);
            ...
            Config.AddSubMenu(new Menu("杩介€愭晫浜簗", "Chase"));
            Config.SubMenu("Chase").AddItem(new MenuItem("ChaseKey", "杩介€愭晫浜簗!").SetValue(new KeyBind("A".ToCharArray()[0], KeyBindType.Press)));
            Config.SubMenu("Chase").AddItem(new MenuItem("UseWChase", "浣跨敤 W").SetValue(true));
            Config.SubMenu("Chase").AddItem(new MenuItem("UseFullComboAfterChase", "鏉熺細鍚庢妧鑳藉叏浜").SetValue(true));
            ...
            Config.SubMenu("LaneClear").AddItem(new MenuItem("UseLaneClearLastHit", "濮嬬粓琛ュ垁").SetValue(false));
            ...
            Config.SubMenu("LaneClear").AddItem(new MenuItem("ManaLaneClear", "娓呯嚎鏈€浣庤摑閲弢").SetValue(new Slider(40, 1, 100)));

            Config.AddSubMenu(new Menu("鎺х嚎", "Freeze"));
            Config.SubMenu("Freeze").AddItem(new MenuItem("FreezeKey", "娣峰悎!").SetValue(new KeyBind(Config.Item("LastHit").GetValue<KeyBind>().Key, KeyBindType.Press)));
            Config.SubMenu("Freeze").AddItem(new MenuItem("UseQFreeze", "浣跨敤 Q 琛ュ垁").SetValue(false));
            Config.SubMenu("Freeze").AddItem(new MenuItem("ManaFreeze", "浣跨敤Q鏈€浣庤摑閲弢").SetValue(new Slider(40, 1, 100)));

            Config.AddSubMenu(new Menu("鏉傞」", "Misc"));
            Config.SubMenu("Misc").AddItem(new MenuItem("PacketCast", "浣跨敤 灏佸寘").SetValue(true));
            Config.SubMenu("Misc").AddItem(new MenuItem("TearExploit", "鍒峰コ绁炰箣娉獆").SetValue(true));
