...
namespace Marksman
{
    internal class PotionManager
    {
        ...
        private void Load()
        {
            ...
            ExtrasMenu.AddSubMenu(new Menu("鑽按缁忕悊", "PotionManager"));

            ExtrasMenu.SubMenu("PotionManager").AddSubMenu(new Menu("鐢熷懡鍊紎", "Health"));
            ExtrasMenu.SubMenu("PotionManager")
                .SubMenu("Health")
                .AddItem(new MenuItem("HealthPotion", "浣跨敤娌荤枟鑽按").SetValue(true));
            ExtrasMenu.SubMenu("PotionManager")
                .SubMenu("Health")
                .AddItem(new MenuItem("HealthPercent", "褰揌P浣庝簬").SetValue(new Slider(30)));

            ExtrasMenu.SubMenu("PotionManager").AddSubMenu(new Menu("榄旀硶鍊紎", "Mana"));
            ExtrasMenu.SubMenu("PotionManager")
                .SubMenu("Mana")
                .AddItem(new MenuItem("ManaPotion", "浣跨敤鍥炶摑鑽按").SetValue(true));
            ExtrasMenu.SubMenu("PotionManager")
                .SubMenu("Mana")
                .AddItem(new MenuItem("ManaPercent", "褰揗P浣庝簬").SetValue(new Slider(30)));
