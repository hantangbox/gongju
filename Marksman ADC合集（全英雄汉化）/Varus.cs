...
namespace Marksman
{
    internal class Varus : Champion
    {
        ...
        public Varus()
        {
            Utils.PrintMessage("Varus loaded! 姹夊寲by浜岀嫍!QQ缇361630847");
        ...
        {
            config.AddItem(
                new MenuItem("UseQC" + Id, "浣跨敤 Q").SetValue(
                    new StringList(new[] {"绂佺敤", "涓嶉檺鍒秥", "W琚姩", "鏈€澶琚姩"}, 3)));
            config.AddItem(
                new MenuItem("UseQMinChargeC" + Id, "Q鏈€浣庤搫鍔泑").SetValue(new Slider(1000, 250, 1500)));
            config.AddItem(new MenuItem("UseWC" + Id, "浣跨敤 W").SetValue(new Slider(3, 1, 3)));
        ...
        {
            config.AddItem(
                new MenuItem("UseQH" + Id, " 浣跨敤 Q").SetValue(
                    new StringList(new[] {"绂佺敤", "涓嶉檺鍒秥", "W琚姩", "鏈€澶琚姩"}, 3)));
            config.AddItem(new MenuItem("UseWH" + Id, "浣跨敤 W").SetValue(new Slider(3, 1, 3)));
        ...
        {
            config.AddItem(new MenuItem("spellDelay" + Id, "娉曟湳寤惰繜").SetValue(new Slider(500, 0, 3000)));
            config.AddItem(
                new MenuItem("CastR" + Id, "鑷姩 R").SetValue(new KeyBind("T".ToCharArray()[0], KeyBindType.Press)));
        ...
        {
            config.AddItem(new MenuItem("DrawQ" + Id, "Q 鑼冨洿").SetValue(new Circle(true, Color.DarkGray)));
            config.AddItem(new MenuItem("DrawE" + Id, "E 鑼冨洿").SetValue(new Circle(true, Color.DarkGray)));
            config.AddItem(new MenuItem("DrawR" + Id, "R 鑼冨洿").SetValue(new Circle(true, Color.DarkGray)));
            config.AddItem(new MenuItem("DrawQC" + Id, "鏈€灏廞鑼冨洿").SetValue(new Circle(true, Color.White)));
            config.AddItem(new MenuItem("DrawRS" + Id, "R: 鍖哄煙鎼滅储").SetValue(new Circle(true, Color.White)));
