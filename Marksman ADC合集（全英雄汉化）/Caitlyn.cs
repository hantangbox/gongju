...
namespace Marksman
{
    internal class Caitlyn : Champion // Base done by Chulbul-Pandey, Drawings and improvements added by Dibes.
    {
        ...
        public Caitlyn()
        {
            Utils.PrintMessage("Caitlyn loaded 姹夊寲by浜岀嫍!QQ缇361630847.");
        ...
        public override bool DrawingMenu(Menu config)
        {
            ...
            config.AddItem(
                new MenuItem("DrawUlt" + Id, "澶ф嫑鎻愮ず鏂囨湰").SetValue(new Circle(true, Color.FromArgb(255, 255, 255, 255))));
            return true;
        }
        ...
        {
            config.AddItem(
                new MenuItem("UltHelp" + Id, "鍙嚮鏉€浣跨敤R").SetValue(new KeyBind("R".ToCharArray()[0],
                    KeyBindType.Press)));
            config.AddItem(
                new MenuItem("UseEQC" + Id, "浣跨敤E-Q浜岃繛").SetValue(new KeyBind("T".ToCharArray()[0],
                    KeyBindType.Press)));
            config.AddItem(
                new MenuItem("Dash" + Id, "E鍚戦紶鏍囨柟鍚憒").SetValue(new KeyBind("Z".ToCharArray()[0], KeyBindType.Press)));
            return true;
        }
