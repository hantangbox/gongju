...
namespace Marksman
{
    ...
    internal class Draven : Champion
    {
        ...
        public Draven()
        {
            ...
            Utils.PrintMessage("Draven loaded 姹夊寲by浜岀嫍!QQ缇361630847.");
        }
        ...
        {
            config.AddItem(new MenuItem("UseQC", "浣跨敤 Q").SetValue(true));
            config.AddItem(new MenuItem("UseWC", "浣跨敤 W").SetValue(true));
            config.AddItem(new MenuItem("UseEC", "浣跨敤 E").SetValue(true));
            config.AddItem(new MenuItem("UseRC", "浣跨敤 R").SetValue(true));
            return true;
        }

        public override bool HarassMenu(Menu config)
        {
            config.AddItem(new MenuItem("UseQH", "浣跨敤 Q").SetValue(true));
            return true;
        }
        ...
        {
            config.AddItem(
                new MenuItem("DrawE", "E 鑼冨洿").SetValue(new Circle(true, Color.FromArgb(100, 255, 0, 255))));
            config.AddItem(
                new MenuItem("DrawR", "R 鑼冨洿(2000 units)").SetValue(
                    new Circle(true, Color.FromArgb(100, 255, 0, 255))));
            config.AddItem(
                new MenuItem("DrawOrbwalk", "鏄剧ず璧颁綅浣嶇疆").SetValue(new Circle(true, Color.Yellow)));
            config.AddItem(new MenuItem("DrawReticles", "鏄剧ず鍏夋爣浣嶇疆").SetValue(new Circle(true, Color.Green)));
            return true;
        }

        public override bool MiscMenu(Menu config)
        {
            config.AddItem(new MenuItem("maxqamount", "Q鍜岃蛋鐮嶅悓鏃惰繘琛寍").SetValue(new Slider(2, 4, 1)));
            config.AddItem(new MenuItem("EGapCloser", "鑷姩E闃茬獊杩泑").SetValue(true));
            config.AddItem(new MenuItem("EInterruptable", "鑷姩E涓柇娉曟湳").SetValue(true));
            config.AddItem(new MenuItem("RManualCast", "鎵嬪姩R(2000 鑼冨洿)"))
                .SetValue(new KeyBind("T".ToCharArray()[0], KeyBindType.Press));
            config.AddItem(new MenuItem("Epeel", "鑷姩E鍑忛€焲").SetValue(true));
            config.AddItem(new MenuItem("CatchRadius", "鎺ユ枾澶村崐寰剕").SetValue(new Slider(600, 200, 1000)));
            return true;
        }
