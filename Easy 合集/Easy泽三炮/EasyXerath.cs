...
namespace EasyXerath
{
    class EasyXerath : Champion
    {
        ...
        protected override void InitializeMenu()
        {
            ...
            Menu.SubMenu("Auto").AddItem(new MenuItem("Auto_q", "浣跨敤 Q").SetValue(false));
            Menu.SubMenu("Auto").AddItem(new MenuItem("Auto_w", "浣跨敤 W").SetValue(false));
            Menu.SubMenu("Auto").AddItem(new MenuItem("Auto_e", "浣跨敤 E").SetValue(true));

            Menu.AddSubMenu(new Menu("缁樺浘", "Drawing"));
            Menu.SubMenu("Drawing").AddItem(new MenuItem("Drawing_q", "浣跨敤 Q").SetValue(new Circle(true, Color.FromArgb(100, 0, 255, 0))));
            Menu.SubMenu("Drawing").AddItem(new MenuItem("Drawing_w", "浣跨敤 W").SetValue(new Circle(true, Color.FromArgb(100, 0, 255, 0))));
            Menu.SubMenu("Drawing").AddItem(new MenuItem("Drawing_e", "浣跨敤 E").SetValue(new Circle(true, Color.FromArgb(100, 0, 255, 0))));
            Menu.SubMenu("Drawing").AddItem(new MenuItem("Drawing_r", "浣跨敤 R").SetValue(new Circle(true, Color.FromArgb(100, 0, 255, 0))));
            Menu.SubMenu("Drawing").AddItem(new MenuItem("Drawing_rdamage", "R 浼ゅ鎸囩ず鍣▅").SetValue(true));

            Menu.AddSubMenu(new Menu("棰濆鍔熻兘", "Misc"));
            Menu.SubMenu("Misc").AddItem(new MenuItem("Misc_wcenter", "閲婃斁 W 灞呬腑").SetValue(true));
            Menu.SubMenu("Misc").AddItem(new MenuItem("Misc_stun", "浣跨敤 E").SetValue<KeyBind>(new KeyBind("T".ToCharArray()[0], KeyBindType.Press)));
            Menu.SubMenu("Misc").AddItem(new MenuItem("Misc_interrupt", "浣跨敤 E 鎵撴柇娉曟湳").SetValue(true));
            Menu.SubMenu("Misc").AddItem(new MenuItem("Misc_ult", "褰搢 ULT 琚縺娲粅 浣跨敤 R ").SetValue(true));
        }
