...
class SkinManager
{
    ...
    public void AddToMenu(ref Menu menu)
    {
        ...
        if (Skins.Count > 0)
        {
            Menu.AddSubMenu(new Menu("鏇存崲鐨偆", "Skin Changer"));
            Menu.SubMenu("Skin Changer").AddItem(new MenuItem("Skin_" + ObjectManager.Player.ChampionName + "_enabled", "浣跨毊鑲よ浆鎹").SetValue(false));
