...
namespace EasyJinx
{
    class EasyJinx : Champion
    {
        ...
        protected override void InitializeMenu()
        {
            ...
            Menu.SubMenu("Auto").AddItem(new MenuItem("Auto_eslow", "浣跨敤 E 鍦ㄥ噺鎱㈡晫浜轰笂").SetValue(true));
            Menu.SubMenu("Auto").AddItem(new MenuItem("Auto_estun", "浣跨敤 E 鍦ㄦ檿鐪╃殑鏁屼汉涓妡").SetValue(true));
            Menu.SubMenu("Auto").AddItem(new MenuItem("Auto_egap", "浣跨敤 E 鍦ㄦ帴杩戦棿闅欎笂").SetValue(true));
            Menu.SubMenu("Auto").AddItem(new MenuItem("Auto_r", "浣跨敤 R").SetValue(true));
            Menu.SubMenu("Auto").AddItem(new MenuItem("Auto_minrange", "鏈€灏弢 R 鑼冨洿").SetValue(new Slider(1050, 0, 1500)));
            Menu.SubMenu("Auto").AddItem(new MenuItem("Auto_maxrange", "鏈€澶 R 鑼冨洿").SetValue(new Slider(3000, 1500, 5000)));

            Menu.AddSubMenu(new Menu("缁樺浘", "Drawing"));
            Menu.SubMenu("Drawing").AddItem(new MenuItem("Drawing_q", "Q 鑼冨洿").SetValue(new Circle(true, System.Drawing.Color.FromArgb(100, 0, 255, 0))));
            Menu.SubMenu("Drawing").AddItem(new MenuItem("Drawing_w", "W 鑼冨洿").SetValue(new Circle(true, System.Drawing.Color.FromArgb(100, 0, 255, 0))));
            Menu.SubMenu("Drawing").AddItem(new MenuItem("Drawing_e", "E 鑼冨洿").SetValue(new Circle(true, System.Drawing.Color.FromArgb(100, 0, 255, 0))));
            Menu.SubMenu("Drawing").AddItem(new MenuItem("Drawing_rdamage", "R 浼ゅ鎸囩ず鍣▅").SetValue(true));

            Menu.AddSubMenu(new Menu("棰濆鍔熻兘", "Misc"));
            Menu.SubMenu("Misc").AddItem(new MenuItem("Misc_qswitch", "琛ュ叺鍒囨崲鏈烘灙").SetValue(true));
            Menu.SubMenu("Misc").AddItem(new MenuItem("Misc_wrange", "W 鏈€灏忚寖鍥磡").SetValue(new Slider(500, 0, (int)Spells.get("W").Range)));
        }
