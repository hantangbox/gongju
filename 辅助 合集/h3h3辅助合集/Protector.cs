    ...
    internal class Protector
    {
        ...
        public static void Init()
        {
            if (!_isInitComplete)
            {
                ...
                Helpers.PrintMessage(string.Format("淇濇姢鍣▅ by h3h3 鍔犺浇!"));
                Console.WriteLine("淇濇姢鍣ㄥ垵濮嬪寲瀹屾垚");
        ...
        {
			Game.PrintChat("<font color=\"#00BFFF\">====== 杞藉叆鎴愬姛:銆怓eeeez姹夊寲 浜岀嫍淇敼銆戣緟鍔╁悎闆唡 ======</font>");
			Game.PrintChat("<font color=\"#00BFFF\">====== 鍖呭惈鑻遍泟====== </font>");
			Game.PrintChat("<font color=\"#00BFFF\">銊х墰澶淬劎鏈哄櫒浜恒劎甯冮殕銊ф湯鏃ャ劎椋庡コ銊уぉ鍚€呫劎鏃ュコ銊ч湶闇层劎鑾敇濞溿劎鍔姫 濞滅編銊х惔濂炽劎瀹濈煶銊ч敜鐭炽劎鏃跺厜銊у鎷夈劎</font>");
			Game.PrintChat("<font color=\"#00BFFF\">鏇村姹夊寲鑴氭湰璇峰姞  L#浜ゆ祦缇わ細361630847</font>");
            Menu = new Menu("杈呭姪鍚堥泦: 淇濇姢鍣▅", "Protector", true);
            ...
            {
                targeted.AddItem(new MenuItem("鎺㈡祴鍣ㄣ劎.閽堝." + ally.ChampionName, ally.ChampionName).SetValue(true));
            }
            ...
            foreach (var ally in ObjectManager.Get<Obj_AI_Hero>().Where(h => h.IsAlly))
            {
                skillshot.AddItem(new MenuItem("鎺㈡祴鍣ㄣ劎.鎶€鑳芥爣绀恒劎." + ally.ChampionName, ally.ChampionName).SetValue(true));
            }
            ...
            foreach (var spell in ProtectorSpells.Where(s => s.ChampionName == ObjectManager.Player.ChampionName))
            {
                ...
                foreach (var ally in ObjectManager.Get<Obj_AI_Hero>().Where(h => h.IsAlly))
                {
                    spellMenu.AddItem(new MenuItem("娉曟湳." + spell.Name + "." + ally.ChampionName, ally.ChampionName).SetValue(true));
                }
            }
            ...
            foreach (var item in ProtectorItems)
            {
                ...
                foreach (var ally in ObjectManager.Get<Obj_AI_Hero>().Where(h => h.IsAlly))
                {
                    itemsMenu.AddItem(new MenuItem("椤圭洰." + item.Name + "." + ally.ChampionName, ally.ChampionName).SetValue(true));
                }
            }

            // cc
            var cc = Menu.AddSubMenu(new Menu("瑙ｉ櫎鎺у埗", "CC"));
            ...
            // misc
            var misc = Menu.AddSubMenu(new Menu("棰濆鍔熻兘", "Misc"));
