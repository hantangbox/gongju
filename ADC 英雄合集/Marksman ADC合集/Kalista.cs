...
namespace Marksman
{
    ...
    internal class Kalista : Champion
    {
        ...
        {
            config.AddItem(
                new MenuItem("JumpTo" + Id, "自动跳跃").SetValue(new KeyBind("T".ToCharArray()[0], KeyBindType.Press)));
            return true;
        }
        ...
        public override bool DrawingMenu(Menu config)
        {
            ...
            config.AddItem(
                new MenuItem("DrawConnMax" + Id, "连接 范围").SetValue(new Circle(false,
                    Color.FromArgb(100, 255, 255, 255))));
            config.AddItem(
                new MenuItem("DrawConnText" + Id, "连接 文本").SetValue(new Circle(false, Color.GreenYellow)));
            config.AddItem(
                new MenuItem("DrawConnSignal" + Id, "连接 信号").SetValue(true));
            config.AddItem(new MenuItem("Dx" + Id, ""));
            config.AddItem(
