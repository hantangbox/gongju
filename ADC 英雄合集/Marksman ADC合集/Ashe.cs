...
namespace Marksman
{
    ...
    internal class Ashe : Champion
    {
        ...
        {
            config.AddItem(new MenuItem("QExploit" + Id, "使用 Q 减速").SetValue(true));
            config.AddItem(new MenuItem("RInterruptable" + Id, "自动 R 中断法术").SetValue(true));
            config.AddItem(new MenuItem("EFlash" + Id, "使用 E 探查 闪现位置").SetValue(true));
            config.AddItem(new MenuItem("RManualCast" + Id, "手动 R(2000 范围)"))
