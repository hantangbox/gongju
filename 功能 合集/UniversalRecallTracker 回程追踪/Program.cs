...
namespace UniversalRecallTracker
{
    public class Program
    {
        ...
        {
            Menu menu = new Menu("回程追踪", "universalrecalltracker", true);
            _x =
            ...
            _textSize = new MenuItem("textSize", "文字大小 (F5 重载)").SetValue(new Slider(15, 5, 50));
            _chatWarning = new MenuItem("chatWarning", "聊天栏 统治").SetValue(false);
            _showOnStart = new MenuItem("showOnStart", "开始 显示").SetValue(true);
        ...
        public void Print(string msg, bool timer = false)
        {
            ...
            s += "<font color='#ff3232'>Universal</font><font color='#BABABA'>鍥炵▼杩借釜鍔犺浇鎴愬姛!姹夊寲by浜岀嫍!QQ缇361630847</font>:</font> <font color='#FFFFFF'>" + msg + </font>";
            Game.PrintChat(s);
        }
                ...
                if (decoded.UnitNetworkId == _hero.NetworkId)
                {
                    switch (decoded.Status)
                    {
                        ...
                        case Packet.S2C.Recall.RecallStatus.RecallFinished:
                            Program.Instance().Notify(_hero.ChampionName + " 宸茬粡鍥炵▼.");
                            _active = false;
                            break;
                        ...
                        case Packet.S2C.Recall.RecallStatus.Unknown:
                            Program.Instance().Notify(_hero.ChampionName + " 宸茬粡<font color='#ff3232'>涓嶈浜嗭紒锛侊紒</font> (" + _hero.Spellbook.GetSpell(SpellSlot.Recall).Name + ")");
                            _active = false;
                            break;
                    }
                }
