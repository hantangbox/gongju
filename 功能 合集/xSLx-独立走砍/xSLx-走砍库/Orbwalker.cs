...
namespace xSLx_Orbwalker
{
	...
	public class xSLxOrbwalker
	{
		...
		public static void AddToMenu(Menu menu)
		{
			...
			menuDrawing.AddItem(new MenuItem("orb_Draw_MinionHPBar_thickness", "^ 琛€鏍煎瘑搴").SetValue(new Slider(1, 1, 3)));
			menuDrawing.AddItem(new MenuItem("orb_Draw_hitbox", "鏄剧ず鎵撳嚮").SetValue(new Circle(true, Color.FloralWhite)));
			menuDrawing.AddItem(new MenuItem("orb_Draw_Lasthit", "鏄剧ず琛ュ叺").SetValue(new Circle(true, Color.Lime)));
			menuDrawing.AddItem(new MenuItem("orb_Draw_nearKill", "闄勮繎鍙ˉ灏忓叺").SetValue(new Circle(true, Color.Gold)));
			...
			menuMisc.AddItem(new MenuItem("orb_Misc_ExtraWindUp", "棰濆缁撴潫鏃堕棿").SetValue(new Slider(80, 200, 0)));
			menuMisc.AddItem(new MenuItem("orb_Misc_AutoWindUp", "鑷姩璁剧疆").SetValue(false));
			menuMisc.AddItem(new MenuItem("orb_Misc_Priority_Unit", "浼樺厛鐩爣").SetValue(new StringList(new[] { "Minion", "Hero" })));
			menuMisc.AddItem(new MenuItem("orb_Misc_Humanizer", "鍚堢悊寤惰繜").SetValue(new Slider(50, 100, 0)));
			...
			{
				var modeCombo = new Menu("璧扮爫", "orb_Modes_Combo");
			...
			menu.AddItem(new MenuItem("xSLx_info2", "鍒朵綔缁剕: xSLx & Esk0r"));
