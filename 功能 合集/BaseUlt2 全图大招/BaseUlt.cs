...
namespace BaseUlt2
{
    ...
    class BaseUlt
    {
        ...
        public BaseUlt()
        {
            (_menu = new Menu("基地大招2", "BaseUlt", true)).AddToMainMenu();
            _menu.AddItem(new MenuItem("showRecalls", "显示回程").SetValue(true));
            _menu.AddItem(new MenuItem("baseUlt", "使用 大招").SetValue(true));
            _menu.AddItem(new MenuItem("extraDelay", "额外 延迟").SetValue(new Slider(0, -2000, 2000)));
            _menu.AddItem(new MenuItem("panicKey", "（恐慌键位）禁用大招").SetValue(new KeyBind(32, KeyBindType.Press))); //32 == space
            _menu.AddItem(new MenuItem("regardlessKey", "无时间限制|保持").SetValue(new KeyBind(17, KeyBindType.Press))); //17 == ctrl

            var teamUlt = _menu.AddSubMenu(new Menu("团队 大招", "TeamUlt"));
            ...
            Game.PrintChat("<font color=\"#1eff00\">鍩哄湴澶ф嫑2 by Beaving</font> - <font color=\"#00BFFF\">閲嶅仛 鍔犺浇鎴愬姛!姹夊寲by浜岀嫍锛丵Q缇361630847</font>");
        }
