    ...
    class Program
    {
        ...
        private static void onGameLoad(EventArgs args)
        {
            try
            {
                ...
                Game.PrintChat(string.Format("<font color='#fb762d'>{0} Loaded v{1}姹夊寲by浜岀嫍!QQ缇361630847</font>", Assembly.GetExecutingAssembly().GetName().Name, Assembly.GetExecutingAssembly().GetName().Version));
        ...
        private static void InitializeMainMenu()
        {
            ...
            Config = new Menu("Dev铔囧コ", "DevCassio", true);
            ...
            Config.SubMenu("Combo").AddItem(new MenuItem("UseIgnite", "浣跨敤 鐐圭噧").SetValue(true));
            Config.SubMenu("Combo").AddItem(new MenuItem("UseAACombo", "浣跨敤 璧癆").SetValue(true));
            Config.SubMenu("Combo").AddItem(new MenuItem("UseRSaveYourself", "浣跨敤 R 淇濇姢鑷繁").SetValue(true));
            Config.SubMenu("Combo").AddItem(new MenuItem("UseRSaveYourselfMinHealth", "浣跨敤 R 瀹夊叏琛€閲弢").SetValue(new Slider(25, 0, 100)));
            ...
            Config.SubMenu("LaneClear").AddItem(new MenuItem("UseELastHitLaneClear", "浣跨敤 E 琛ュ垁").SetValue(true));
            Config.SubMenu("LaneClear").AddItem(new MenuItem("LaneClearMinMana", "娓呯嚎鏈€浣庤摑閲弢").SetValue(new Slider(25, 0, 100)));
            ...
            Config.SubMenu("Gapcloser").AddItem(new MenuItem("RAntiGapcloser", "R 闃叉绐佽繘").SetValue(true));
            Config.SubMenu("Gapcloser").AddItem(new MenuItem("RInterrupetSpell", "R 涓柇娉曟湳").SetValue(true));
            Config.SubMenu("Gapcloser").AddItem(new MenuItem("RAntiGapcloserMinHealth", "R 绐佽繘鑰呮渶浣庤閲弢").SetValue(new Slider(60, 0, 100)));

            Config.AddSubMenu(new Menu("鏉傞」", "Misc"));
            Config.SubMenu("Misc").AddItem(new MenuItem("PacketCast", "浣跨敤鍙嶅悜E(灏佸寘)").SetValue(true));

            Config.AddSubMenu(new Menu("E璁剧疆", "Legit"));
            Config.SubMenu("Legit").AddItem(new MenuItem("PlayLegit", "E璁剧疆:").SetValue(false));
            Config.SubMenu("Legit").AddItem(new MenuItem("DisableNFE", "绂佺敤鍙嶅悜E").SetValue(true));
            Config.SubMenu("Legit").AddItem(new MenuItem("LegitCastDelay", "浣跨敤E寤惰繜").SetValue(new Slider(500, 0, 1500)));

            Config.AddSubMenu(new Menu("R璁剧疆", "Ultimate"));
            Config.SubMenu("Ultimate").AddItem(new MenuItem("UseAssistedUlt", "澶ф嫑鎻村姪闃熷弸").SetValue(true));
            Config.SubMenu("Ultimate").AddItem(new MenuItem("AssistedUltKey", "寮哄埗R").SetValue((new KeyBind("R".ToCharArray()[0], KeyBindType.Press))));
            Config.SubMenu("Ultimate").AddItem(new MenuItem("BlockUlt", "闃叉绌哄ぇ").SetValue(true));
            Config.SubMenu("Ultimate").AddItem(new MenuItem("UseUltUnderTower", "濉斾笅鑷姩R鏁屼汉").SetValue(true));
            Config.SubMenu("Ultimate").AddItem(new MenuItem("UltRange", "浣跨敤R鑼冨洿").SetValue(new Slider(700, 0, 850)));
            Config.SubMenu("Ultimate").AddItem(new MenuItem("RMinHit", "浣跨敤R浜烘暟").SetValue(new Slider(2, 1, 5)));
            Config.SubMenu("Ultimate").AddItem(new MenuItem("RMinHitFacing", "浣跨敤R姝ｅ浜烘暟").SetValue(new Slider(1, 1, 5)));
            ...
            Config.SubMenu("Drawings").AddItem(new MenuItem("EDamage", "鏄剧ずE鐨勪激瀹硘").SetValue(true));
